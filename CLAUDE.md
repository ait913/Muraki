...
├── projects/<slug>/                                # AI 管理プロジェクト本体 (git repo 必須)
│   └── .designs/<YYYYMMDD>-<feature-slug>.md       # 設計docはここ
└── worktrees/<project>-<feature-slug>/             # 並列作業用worktree (.gitignore 推奨)
```
...
書く前に**どこに置くか**を決める。3層あり、各層の責務は被らせない。
...
各プロジェクトの `.gitignore` に worktree 経路の混入を防ぐ記述は不要 (worktreeは別パス)。ただし Muraki ルートで一括管理したい場合は `Muraki/.gitignore` に `worktrees/` を入れる。
