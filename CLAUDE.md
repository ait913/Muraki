...
判断の難所は「不安を感じたら」でなく**観測可能なシグナル**で Touri に上げる。Leader のモデルが変わっても発火するよう、シグナルは判断でなくイベントで定義する。形式は AskUserQuestion (要点 + Leader 推奨 + 選択肢 2-4)。
...
「既存破損だから無視」は台帳と照合して初めて言える。各 PJ は既知の失敗テストを分類付き (テスト陳腐化 / 環境依存 / **未分類**) で `Muraki/projects/<slug>/.knowledge/known-failures.md` に持つ。**未分類の失敗を残したままのマージは不可** — 本番バグが「ベースライン失敗の山」に隠れた実績あり (sessions/2026-06-11)。台帳が無い PJ は次に触るとき棚卸しして作る。
...
- **目的** (1-3行): なぜ作るか
- **UI/UX** (該当時): 画面レイアウト・コンポーネント構成・遷移。観点は `knowledge/pattern/ui-ux-design-perspectives.md` (汎用層) を通す。PJ の DESIGN.md があればそちらが正典
- **データモデル**: スキーマ・型定義
