...
- **QR を後送りで足すとき (2026-08-21, bloom): 既存の招待 URL (上記 inviteCode ベース) をそのままエンコードするだけにし、QR 専用の新しい URL 種別/トークンは発行しない。** URL 種別を増やすと失効・再発行の規則が種別ごとに二重化し「片方は生きているのにもう片方は死んでいる」不整合を生む。招待の入口 (ヘッダーの専用アイコン・設定画面の行、等) を複数 UI に増やす場合も、各入口は同一の招待シート/画面 1 つを開くだけにして実装を 1 本化する (3 箇所で招待フローを別実装すると挙動が割れる)
